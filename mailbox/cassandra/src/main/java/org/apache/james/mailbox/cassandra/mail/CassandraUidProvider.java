...
public class CassandraUidProvider implements UidProvider {
    ...
    public MessageUid nextUid(MailboxSession mailboxSession, Mailbox mailbox) throws MailboxException {
        ...
        if (! findHighestUid(mailboxId).isPresent()) {
            Optional<MessageUid> optional = tryInsertUid(mailboxId, com.google.common.base.Optional.absent());
    ...
    private Optional<MessageUid> tryInsertUid(CassandraId mailboxId, com.google.common.base.Optional<MessageUid> uid) {
        MessageUid nextUid = uid.transform(MessageUid::next).or(MessageUid.MIN_VALUE);
