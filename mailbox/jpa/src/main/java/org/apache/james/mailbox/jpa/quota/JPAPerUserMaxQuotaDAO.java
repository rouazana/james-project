...
public class JPAPerUserMaxQuotaDAO {
    ...
    public void setMaxStorage(QuotaRoot quotaRoot, Optional<QuotaSize> maxStorageQuota) {
        ...
        entityManager.merge(new MaxUserStorage(quotaRoot.getValue(), quotaValueToLong(maxStorageQuota)));
        entityManager.getTransaction().commit();
    }

    public void setMaxMessage(QuotaRoot quotaRoot, Optional<QuotaCount> maxMessageCount) {
        entityManager.getTransaction().begin();
        entityManager.merge(new MaxUserMessageCount(quotaRoot.getValue(), quotaValueToLong(maxMessageCount)));
        entityManager.getTransaction().commit();
    }

    public void setDefaultMaxStorage(Optional<QuotaSize> defaultMaxStorage) {
        entityManager.getTransaction().begin();
        entityManager.merge(new MaxDefaultStorage(quotaValueToLong(defaultMaxStorage)));
        entityManager.getTransaction().commit();
    }

    public void setDefaultMaxMessage(Optional<QuotaCount> defaultMaxMessageCount) {
        entityManager.getTransaction().begin();
        entityManager.merge(new MaxDefaultMessageCount(quotaValueToLong(defaultMaxMessageCount)));
        entityManager.getTransaction().commit();
    }
