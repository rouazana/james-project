...
public class FileMailQueueFactory implements MailQueueFactory, LogEnabled {
    ...
    private MailQueueActionFactory mailQueueActionFactory;
    private FileSystem fs;
    private Logger log;
    private boolean sync = true;

    @Inject
    public FileMailQueueFactory(FileSystem fs, MailQueueActionFactory mailQueueActionFactory) {
        this.fs = fs;
    ...
    public MailQueue getQueue(String name) {
        ...
        if (queue == null) {
            synchronized (queues) {
                try {
                    queue = new FileMailQueue(mailQueueActionFactory, fs.getFile("file://var/store/queue"), name, sync, log);
