...
public class FileMailQueueFactory implements MailQueueFactory, LogEnabled {
    ...
    private MailQueueActionProvider mailQueueActionProvider;
    private FileSystem fs;
    private Logger log;
    private boolean sync = true;

    @Inject
    public FileMailQueueFactory(FileSystem fs, MailQueueActionProvider mailQueueActionProvider) {
        this.fs = fs;
    ...
    public MailQueue getQueue(String name) {
        ...
        if (queue == null) {
            synchronized (queues) {
                try {
                    queue = new FileMailQueue(mailQueueActionProvider, fs.getFile("file://var/store/queue"), name, sync, log);
